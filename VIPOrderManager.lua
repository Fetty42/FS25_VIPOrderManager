					...
					if orderEntry ~= nil then
						...
						if cluster.age >= orderEntry.neededAgeInMonths and cluster.health >= 75 then
							...
							if numAninmalsToSell > 0 then
								...
								g_currentMission:addMoney(sellPrice, g_currentMission:getFarmId(), MoneyType.SOLD_ANIMALS, true, true);
...
function VIPOrderManager:AbortCurrentVIPOrder()
    ...
    dialog.target:setText(msgText)
    dialog.target:setTitle(g_i18n:getText("VIPOrderManager_DlgTitel_AbortCurrentVIPOrder"))
    -- dialog.target:setDialogType(Utils.getNoNil(args.dialogType, DialogElement.TYPE_QUESTION))
    dialog.target:setCallback(VIPOrderManager.reactToDialog_AbortCurrentVIPOrder, self)
	...
	-- DialogHelper.showYesNoDialog({title = g_i18n:getText("VIPOrderManager_DlgTitel_AbortCurrentVIPOrder"), text = msgText, callback = VIPOrderManager.reactToDialog_AbortCurrentVIPOrder, target = self})
end
...
function VIPOrderManager:reactToDialog_AbortCurrentVIPOrder(yes)
	...
	if yes and VIPOrderManager.VIPOrders[1] ~= nil then
		...
		g_currentMission:addMoney(sumPayout, g_currentMission:getFarmId(), MoneyType.MISSIONS, true, true);
		g_currentMission:addMoney(sumAbortFee, g_currentMission:getFarmId(), MoneyType.MISSIONS, true, true);
...
function VIPOrderManager:RestockVIPOrders()
	...
	if g_currentMission:getIsClient() and g_currentMission:getFarmId() > 0 then
		-- new VIPOrder	
		local orderLevel = 0
	...
	if g_currentMission.productionChainManager.productionPoints ~= nil then
		for _, productionPoint in pairs(g_currentMission.productionChainManager.productionPoints) do
			...
			for fillTypeId, fillLevel in pairs(productionPoint.storage.fillLevels) do
                ...
                local fillTypeName = g_fillTypeManager.fillTypes[fillTypeId].name
				local fillTypeTitle = g_fillTypeManager.fillTypes[fillTypeId].title
				local isInput = false
...
function VIPOrderManager:CalculateOwnFieldArea()
	...
	local farmlands = g_farmlandManager:getOwnedFarmlandIdsByFarmId(g_currentMission:getFarmId())
	...
	return MyTools:round(fieldAreaSum, 2)
end
...
function VIPOrderManager:GetRelevantFillTypes(relevantFillTypes)
	...
	if dbPrintfOn then
		...
		for index, possibleFT in pairs(possibleFillTypes) do
			...
			dbPrintf("  - %-50s: priceMax=%5.3f | pricePerLiter=%5.3f | Stations=%s", tempNameOutput, possibleFT.priceMax, possibleFT.pricePerLiter, possibleFT.acceptingStationsString)
		end
		dbPrintf("")
	end
...
function VIPOrderManager:addAllSellableFillTypes(possibleFillTypes)
	...
	for _, station in pairs(g_currentMission.storageSystem.unloadingStations) do
		...
		dbPrintf("Station: getName=%s | typeName=%s | categoryName=%s | isSellingPoint=%s | uniqueId=%s | placeable.ownerFarmId=%s | g_currentMission:getFarmId()=%s",
			placeable:getName(), tostring(placeable.typeName), tostring(placeable.storeItem.categoryName), tostring(station.isSellingPoint), placeable.uniqueId, placeable.ownerFarmId, g_currentMission:getFarmId())
...
function VIPOrderManager:MakePayout(orderEntries)
	...
	g_currentMission:addMoney(payoutTotal, g_currentMission:getFarmId(), MoneyType.MISSIONS, true, true);
end
...
function VIPOrderManager:update(dt)
	...
	if VIPOrderManager.updateDelta > VIPOrderManager.updateRate and VIPOrderManager.InitDone then
		...
		if g_currentMission:getIsClient() and g_gui.currentGui == nil and g_currentMission:getFarmId() > 0 then
	...
	for _, vipOrder in pairs(VIPOrderManager.VIPOrders) do
		...
		for _, orderEntry in pairs(vipOrder.entries) do
			...
			if orderEntry.targetStation ~= nil then
				setXMLString(xmlFile, entryKey.."#targetStationUniqueId", orderEntry.targetStation.owningPlaceable.uniqueId)
				...
				if hasXMLProperty(xmlFile, orderKey) then
					...
					while true do
						...
						if hasXMLProperty(xmlFile, entryKey) then
							...
							local targetStationUniqueId = getXMLString(xmlFile, entryKey.."#targetStationUniqueId")
							local targetStationName = getXMLString(xmlFile, entryKey.."#targetStationName_OnlyAsInfo")
							-- dbPrintf("loadSettings: %s | %s", targetStationUniqueId, targetStationName)
							local targetStation = VIPOrderManager:getStationByUniqueId(targetStationUniqueId)
...
function VIPOrderManager:getStationByUniqueId(targetStationUniqueId)
    dbPrintHeader("VIPOrderManager:getStationByUniqueId()")

	for _, station in pairs(g_currentMission.storageSystem.unloadingStations) do
		if station.owningPlaceable ~= nil and station.owningPlaceable.uniqueId == targetStationUniqueId then
			return station
		end
	end
	return nil
...
function VIPOrderManager:GetAnimalTitleByFillTypeIdx(fillTypeIdx, neededAgeInMonths)
	local animalFillType = g_fillTypeManager:getFillTypeByIndex(fillTypeIdx)
    local animalSubType = g_currentMission.animalSystem.fillTypeIndexToSubType[fillTypeIdx]
	local animalStoreTitle = animalFillType.title
	local animalAge = neededAgeInMonths == nil and 0 or neededAgeInMonths
	
	for i, visual in pairs(animalSubType.visuals) do
		if animalAge >= visual.minAge and visual.store.name ~= nil then
